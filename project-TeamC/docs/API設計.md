...
> FastAPIが実装内容をもとにOpenAPIスキーマを自動生成する。
> 開発環境ではSwagger UI（http://localhost:8000/docs）からAPI仕様と動作を確認できる。
...
| 操作         | student            | teacher          |
| ------------ | ------------------ | ---------------- |
| 投稿一覧取得 | 自分の投稿のみ可能 | 生徒全員分が可能 |
| 投稿詳細取得 | 自分の投稿のみ可能 | 生徒全員分が可能 |
| 投稿作成     | 可能               | 不可             |
| 投稿更新     | 自分の投稿のみ可能 | 不可             |
| 投稿削除     | 自分の投稿のみ可能 | 不可             |
...
| title              | APIでは必須。255文字以内。画面で未入力の場合は、フロントエンドが投稿種別に応じて「日報」または「進捗報告」を設定して送信 |
| content            | 必須                                                                                                                     |
| blockers           | 任意                                                                                                                     |
| nextAction         | 任意                                                                                                                     |
...
- MVPでは、`teacher` による投稿の作成・編集・削除は対象外とする
- 講師用投稿一覧画面には `/teacher` を使用する
- `/admin/reports` のような管理専用APIは作成しない
- 絞り込み、確認ステータス管理などの詳細な管理機能はMVP対象外とする
